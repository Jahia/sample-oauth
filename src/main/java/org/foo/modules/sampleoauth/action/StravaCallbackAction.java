...
public class StravaCallbackAction extends Action {
    ...
    private static final String NAME = "stravaOAuthCallbackAction";
        ...
        if (parameters.containsKey("code") && parameters.containsKey(JahiaOAuthConstants.STATE)) {
            ...
            isAuthenticate = JCRTemplate.getInstance().doExecuteWithSystemSession(new JCRCallback<Boolean>() {
                ...
                public Boolean doInJCR(JCRSessionWrapper systemSession) throws RepositoryException {
                    ...
                    try {
                        jahiaOAuthService.extractAccessTokenAndExecuteMappers(jahiaOAuthNode, stravaConnectorImpl.getServiceName(), token, state);
                        return true;
                    } catch (Exception ex) {
