...
<template:addResources>
    <script>
        function connectToStrava${fn:replace(currentNode.identifier, '-', '')}() {
            var popup = window.open('', "Strava Authorization", "menubar=no,status=no,scrollbars=no,width=1145,height=725,modal=yes,alwaysRaised=yes");
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '<c:url value="${url.base}${renderContext.site.home.path}"/>.connectToStravaAction.do');
...
<c:choose>
    <c:when test="${tagType eq 'button'}">
        <button class="google-btn custom-btn-theme ${cssClass}" type="button"
                <c:if test="${not renderContext.editMode}"> onclick="connectToStrava${fn:replace(currentNode.identifier, '-', '')}()" </c:if>
    ...
    <c:otherwise>
        <a href="#" class="google-btn custom-btn-theme ${cssClass}"
                <c:if test="${not renderContext.editMode}"> onclick="connectToStrava${fn:replace(currentNode.identifier, '-', '')}();return false;" </c:if>
                <c:if test="${not empty htmlId}"> id="${htmlId}"</c:if>
           <c:if test="${renderContext.editMode}">disabled</c:if> >
                <span class="custom-icon-svg">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
